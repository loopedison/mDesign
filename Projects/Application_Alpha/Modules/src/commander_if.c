...
uint32_t Commander_If_AutoUpload(void const * argument)
{
  ...
  if(tickNew - tickLst >= *(uint16_t *)hStorageMsgData.xUserConf.xPeriod)
  {
    ...
    if(hStorageMsgData.xUserConf.xOE[0] == 0xff)
    {
      ...
      atxMsg[iCnt++] = 0XFF;
      atxMsg[iCnt++] = 0X05;
      xTemp = tsensor.xData.xSpeed*(tsensor.xParam.xSpdMax-tsensor.xParam.xSpdMin)/100;
      if(xTemp < 5) { xTemp = 0; }
      atxMsg[iCnt++] = (xTemp&0x00ff);
      atxMsg[iCnt++] = (xTemp&0xff00)>>8;
      xTemp = (50)*tsensor.xData.xYaw/100;    //range [-50 ~ +50]
      atxMsg[iCnt++] = (xTemp&0x00ff);
      atxMsg[iCnt++] = (xTemp&0xff00)>>8;
      xTemp = tsensor.xData.xButton;
      atxMsg[iCnt++] = (xTemp&0x00ff);
      xCRC=CRC16_MODEBUS(atxMsg,iCnt);
      atxMsg[iCnt++] = (xCRC&0x00ff);
      atxMsg[iCnt++] = (xCRC&0xff00)>>8;
      /* Transfer via usb */
