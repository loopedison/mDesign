...
{
  .xRatePeriod[0]     = (uint8_t)((PARAM_PERIOD)),
  .xRatePeriod[1]     = (uint8_t)((PARAM_PERIOD)>>8),
};
...
#define ADDR_USER_DATA_RATE         (0X20)
#define ADDR_USER_DATA_DIR          (0X21)
...
uint32_t Commander_If_Read(uint32_t pAddr, uint8_t *pBuff, uint32_t pLen)
{
  ...
  else if(pAddr == ADDR_USER_DATA_RATE)
  {
    if(pLen == sizeof(int8_t))
    {
      int8_t xDir = (int8_t)tsensor.xData.xRate;
      memcpy(pBuff, &xDir, sizeof(int8_t));
      errStatus = CMD_OK;
    }
    else
    {
  ...
  else if(pAddr == ADDR_USER_DATA_DIR)
  {
    if(pLen == sizeof(int8_t))
    {
      int8_t xDir = (int8_t)tsensor.xData.xDirection;
      memcpy(pBuff, &xDir, sizeof(int8_t));
      errStatus = CMD_OK;
    }
    else
...
uint32_t Commander_If_AutoUpload(void const * argument)
{
  ...
  if(hStorageMsgData.xUserConf.xOE[0] == 0x01)
  {
    if(tickNew - tickLst >= *(uint16_t *)hStorageMsgData.xUserConf.xPeriod)
    {
      ...
      atxMsg[iCnt++] = tsensor.xData.xRate;
      atxMsg[iCnt++] = tsensor.xData.xDirection;
