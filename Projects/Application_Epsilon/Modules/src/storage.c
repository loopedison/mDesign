...
Storage_StatusTypeDef Storage_Init(void)
{
  ...
  memset(&hStorageInfo, 0xff, sizeof(Storage_InfoDef));
  status = Storage_ReadInfo(STORAGE_HANDLE, &hStorageInfo);
  if(status != STORAGE_OK) { return (status); }
  memset(&hStorageIndex, 0xff, sizeof(uint32_t));
  status = Storage_ReadIndex(STORAGE_HANDLE, &hStorageIndex);
  if(status != STORAGE_OK) { return (status); }
  memset(&hStorageMsgData, 0xff, sizeof(Storage_MsgDataTypeDef));
...
Storage_StatusTypeDef Storage_WriteIndexMsgData(Storage_TypeDef *pStorage, uint32_t index, Storage_MsgDataTypeDef *msgData)
{
  ...
  if(hStorageIndex != index)
  {
    if(index >= STORAGE_MSG_ZISE)
    {
      index = 0;
      StorageFlash_Erase();
      Storage_WriteInfo(STORAGE_HANDLE, (Storage_InfoDef *)&hStorageInfo);
    }
    hStorageIndex = index;
